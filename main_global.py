#!/usr/bin/env python3
"""
Meeting Monitor (Global Version)
===============================
Detects active Microsoft Teams calls, records audio via system microphone,
transcribes with Whisper (local), summarizes with LLM API, and emails
...
class AudioRecorder:
    ...
    def start(self, label: str = "") -> None:
        ...
        name  = f"meeting_{ts}" if not label else f"meeting_{ts}_{label}"
...
class Transcriber:
    ...
    def transcribe(self, audio_path: Path) -> str:
        ...
        # Generalized prompt for Whisper
        prompt = "This is a meeting transcript. It may contain technical terminology and multiple languages."
...
Today is {today}. Below are transcripts from {total} recorded meeting(s).
...
- **Owner:** Identify the specific name of the person responsible.
...
Be concise and professional. Use plain text bullets."""
...
class MeetingMonitor:
    ...
    def __init__(self):
        ...
        self.summarizer = Summarizer(self.cfg["gemini_api_key"])
    ...
    def _monitor_loop(self):
        logging.info("👀 Monitoring for calls…")
        while True:
            try:
                self._tick()
            except Exception as exc:
